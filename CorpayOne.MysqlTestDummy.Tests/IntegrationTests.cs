...
namespace CorpayOne.MysqlTestDummy.Tests
{
    public class IntegrationTests : IClassFixture<DatabaseFixture>
    {
        ...
        [Fact]
        public void SupportsQuadCompositeKey()
        {
            /*
             * This test works by luck more than anything. The table under test has 4 primary key columns:
             * - Id: auto incremented int
             * - UserId: int
             * - Event: int
             * - ApplicationType: int
             *
             * The current logic to handle this is wrong because the "Id" field is manually defined and inserted but it should be
             * left to be autogenerated otherwise there is a 1 in 499 chance we will pick a column that conflicts. (Id range for random
             * generation is defined as 1-500). It passes for now and in our use-case we're unlikely to call it often enough that this
            ...
            for (var i = 0; i < 200; i++)
            {
                Dummy.CreateId(
                    ...
                    "MaintenanceEvents",
                    new DummyOptions<int>()
                        .WithColumnValue("Token", Guid.NewGuid().ToString()));
            }
        }
    }
}
